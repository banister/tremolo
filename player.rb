...
class Player < Actor
    ...
    def do_controls(control_id=nil)
        ...
        #modify angle
        if @controls[:right] == control_id then da = 1.0; @tank.effects.play_effect(:turret,0.2); end
        if @controls[:left] == control_id then da = -1.0; @tank.effects.play_effect(:turret,0.2); end
    ...
    def projectile_collided_with(thing, projec)
        ...
        if SampleActor === thing then @health += 5; end
    end
    ...
    def do_collision(thing)
        ...
        new_anim = @anim_group.new_entry(thing.x, thing.y, ImageSystem.new(@window))

        new_anim.anim.make_animation(:blast,@blast,:timing => 0.06,:loop => false,:hold => false)

        new_anim.anim.load_animation(:blast)
    ...
    def draw(ox,oy)
        ...
        # draw other animations (explosions etc)
